...
export function Fretboard({ root, scaleId, showIntervals = false, startFret = 0, displayFrets = 12, viewMode = "full" }: Props) {
  ...
  return (
    <div className="rounded-3xl border border-zinc-800 bg-zinc-950 p-3 shadow-2xl shadow-black/30 sm:p-4">
      ...
      <div className="overflow-hidden rounded-2xl border border-zinc-800 bg-gradient-to-r from-amber-950/40 via-zinc-900 to-amber-950/25 p-2 sm:p-3">
          ...
          return (
            <div key={`${label}-${visualIndex}`} className="grid items-center gap-0.5 border-t border-zinc-700/80 py-1.5 last:border-b" style={{ gridTemplateColumns: `28px repeat(${frets.length}, minmax(0, 1fr))` }}>
              ...
              {rowNotes.map((note) => {
                ...
                return (
                  <div key={`${note.stringNumber}-${note.fret}`} className="relative flex h-9 items-center justify-center border-l border-zinc-700/70 bg-zinc-950/20 first:border-l-4 first:border-zinc-200/80 sm:h-10">
                    ...
                    {showScaleNote ? (
                      ...
                      <span className="relative z-10 rounded-full bg-zinc-800/80 px-1.5 py-0.5 text-[10px] font-bold text-zinc-200 sm:text-xs">{note.note}</span>
                    ) : (
                      <span className="relative z-10 text-[9px] font-semibold text-zinc-500">·</span>
                    )}
                  </div>
                );
              })}
